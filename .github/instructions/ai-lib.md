...
- **开始**: **①先同步** (问答/只读轮次跳过; **首个执行动作 —— 改文件 / 跑测试 / 任何 git 写操作 —— 之前必须完成**) —— `git fetch gitee develop` (远端与分支名**必须写**), `git ls-remote gitee develop` 对比本地 HEAD 确认不落后 (`status -sb` 是快照); 纯落后且工作区干净 → `git merge --ff-only FETCH_HEAD`; 树脏 → **停下报告, 禁止自行清理**; **禁止在落后分支上动手** (细则见 `AGENTS.md`「会话协议 · 开始」与「⚠️ 环境硬约束: Git 操作」)。再 ②跑 `python .agents/skills/memory-bank/scripts/gen_active_recent.py` 看 `memory-bank/activeContext/` 切片(会话滚动状态) 与 `memory-bank/README.md` 路由表, 按任务深入对应文档; 已有 `memory-bank/tasks/` 档案的任务从档案续作。
- **收尾**: 按 skill 的 5 步 DoD 执行 (更新 activeContext 切片 + 已完成条目迁出 / 命中阈值立档 / 回写事实与 README / 跑闸门 pytest / 记新坑); 守卫 `tests/test_memory_bank.py` 校验档案一致性。
