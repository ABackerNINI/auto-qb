...
- **开始**: **①先拉取远程分支** —— `git remote -v` 确认主线远端 (Gitee), 再 `git pull --rebase <remote> develop` (分支名**必须写**), 确认不落后后再改代码; **禁止在落后的分支上动手** (细则见 [AGENTS.md](../AGENTS.md)「会话协议 · 开始」与「提交 / PR」)。再 ②读 `memory-bank/activeContext.md` (当前焦点) + `memory-bank/README.md` 路由表; 已有 `tasks/` 档案的任务从其档案续作。
