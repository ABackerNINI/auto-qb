...
预检这时会同时给「落后 N 个提交（STOP，commit / push 两阶段都是）」和「工作区脏（WARN）」。
**同步必须先于提交** —— 顺序反过来就会掉进"`merge --ff-only` 必然失败"的坑（见下）。
旧版因拦截层毁库禁用 stash / rebase，该问题 2026-09-25 已修复；`--phase commit` 对落后曾只报
WARN（放行"先提交后合流"），**2026-09-26 起改为 STOP** —— 那个顺序正是收尾回写撞 baseline 的根因（见下节）。
落后时闸门也随之不跑：合并远端后反正要重跑提交预检，先跑全量测试纯属白跑。
...
## 先合并远端，再收尾回写（2026-09-26 定稿 —— 「baseline 总是撞」的解法）

不少项目要求提交时同步更新知识库、文档或重建索引。**回写件（baseline.md / activeContext 切片 / 各 _index）
是全体 clone 的收尾 DoD 都要写的最热写点** —— 在陈旧基线上回写，远端一合流必撞。所以收到「提交」的固定顺序是：

```
S0 my-commit-flow.sync 预检（只读）  → 落后 / 分叉即按「同步路径」合并远端
...
- 回写件**属于本次提交的一部分**，要在 S3 暂存前做完并一起暂存；不推完再补一笔
  （已推送的提交不能 amend + 强推，补交就意味着第二笔提交、再走一次流水线）。
- 提交信息取**主导意图**（如 ✨ / 🐛），概述里带一句"并回写知识库"，不另起一条。
- 回写量本身很大、或属于另一件事（顺手修的无关文档）时，按常规拆成独立一笔。

## 闸门在流水线里跑几次

预检有四个触发点（S0 只读预检不跑闸门），闸门实际**只跑两次**：

```
S0 my-commit-flow.sync（--check-started）  → 只读，不跑闸门；落后在此按「同步路径」合并远端
