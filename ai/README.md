...
- **测试**: `pytest` + `tests/helpers.py` 全 Fake (无需真实 qBittorrent)。当前基线: **722 passed + 1 skipped, 覆盖率 87%**(ui.py GUI 本体真机冒烟; WEB UI 端到端 = 后端单测 + 临时 Fake 服务浏览器冒烟), 全量运行约 6 秒。
...
.venv/Scripts/python.exe -m pytest tests -q            # 全量 722 项 + 1 skip, ~6s
