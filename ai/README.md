...
- **测试**: `pytest` + `tests/helpers.py` 全 Fake (无需真实 qBittorrent; 另有 `FakeQbServer` 本地假 HTTP 服务, 供真实 `qbittorrent-api`/requests 栈的集成测试)。当前基线: **849 passed, 0 skipped, 覆盖率 91%**(ui.py GUI 本体真机冒烟; WEB UI 端到端 = 后端单测 + 临时 Fake 服务浏览器冒烟), 全量运行约 11 秒(含覆盖率报表)。
