...
- **核心设计**: `QbManager` 由 6 个 mixin 组合; 所有工作 (种子刷新/规则/内置功能/校验轮询) 统一为带内置 interval 的任务, 进单一时间优先堆; `TorrentSync` 走 qB `/api/v2/sync/maindata` rid 增量同步(只收变化种子的变化字段) → `TorrentStore` 合并入快照 + 惰性缓存; `QbApi` Facade写后同步快照; 主循环单线程, 唯一修改队列与 state_file 的线程。
- **测试**: `pytest` + `tests/helpers.py` 全 Fake (无需真实 qBittorrent)。当前基线: **784 passed + 1 skipped, 覆盖率 90%**(ui.py GUI 本体真机冒烟; WEB UI 端到端 = 后端单测 + 临时 Fake 服务浏览器冒烟), 全量运行约 4 秒。
...
.venv/Scripts/python.exe -m pytest tests -q            # 全量 756 项 + 1 skip, ~7s
