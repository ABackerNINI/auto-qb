...
- 站点配置覆盖全局: `load_tracker_hr(spec, global_hr)` 站点字段优先全局fallback; `remove_similar_tags` 同理。合并发生在加载时, 运行期只用合并后的值。
