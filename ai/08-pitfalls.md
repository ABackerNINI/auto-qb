...
- **本地 qB 关闭 requests `trust_env`** (2026-09-13): `_disable_env_lookup_for_local` 对 `127.0.0.1`/`localhost` 关闭 Session 的 `trust_env`(跳过每请求 `get_environ_proxies`/`get_netrc_auth`); 远程域名保留默认(企业代理/netrc 需求)。依赖库私有属性 `Client._session`, 已 try/except 兑底; **Session 被库内部重置(重试/重连)后会回到默认**, 但每次 `connect()` 都会重新应用。
