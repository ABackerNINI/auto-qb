...
6. **WEB UI**(`web.enabled`): 浏览器访问的辅种管理界面 —— 按辅种分组实时查看(站点状态徽章/速度/上传量)、组级控制(暂停/开始/强制汇报/删除)、种子搜索(按种子名/文件列表筛选辅种组, 未归组命中单独列出)、**图形化配置编辑**(每个配置项均可增删改, 含站点/规则集/限速曲线的结构化编辑, 无直接编辑 YAML 模式; 只读 YAML 预览供核对, 保存后热重载生效); Vue 3 本地托管, Bearer 密钥鉴权(首启随机生成); 前端按 `rid` 增量轮询(服务端视图版本未变时不回传分组数据、不重渲染) + 页面隐藏停轮询 + 无变化自适应退避(2s→5s→10s)
