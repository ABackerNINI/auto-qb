...
- **HR (Hit and Run)**: 站点惩罚规则 — 下载后必须在规定时间内做种达到要求(时长/分享率), 否则受罚。本项目自动给"触发 HR"的种子打标 (如 `!!HR3D!!`), 给"已满足 HR"的打标 (如 `--HR3D--`)。触发条件 = 下载比例 (如 80%) 或下载量绝对值; 未达触发量的种子下载完成也视为触发 (小种子边界, 2026-09-12)。
...
6. **WEB UI**(`web.enabled`): 浏览器访问的辅种管理界面 —— 按辅种分组实时查看(站点状态徽标/速度/上传量)、组级控制(暂停/开始/强制汇报/删除)、种子搜索(按种子名/文件列表筛选辅种组, 未归组命中单独列出)、**H&R 栏**(未满足做种时长/分享率的成员数 / 已触发 HR 成员数)与明细表 `实际 / 要求` 对照(做种时长、分享率, 未达/已达分别着色)、顶栏**今日累计上传/下载**与**命中/实际限速对照**(不一致时显示两者并给出原因, 需启用限速曲线)、默认按组内**最近添加时间**排序(列头点击 3 次恢复默认)、**图形化配置编辑**(每个配置项均可增删改, 含站点/规则集/限速曲线的结构化编辑, 无直接编辑 YAML 模式; 只读 YAML 预览供核对, 保存后热重载生效); Vue 3 本地托管, Bearer 密钥鉴权(首启随机生成); 前端按 `rid` 增量轮询(服务端视图版本未变时不回传分组数据、不重渲染) + 页面隐藏停轮询 + 无变化自适应退避(2s→5s→10s)
7. **主动通知**: WARNING 及以上日志推送平台原生通知(Windows toast/Linux/macOS, 零依赖); 可配级别/免打扰时段/节流
8. **导出**: `--export-yaml` 从现有种子 tracker 生成配置模板 (尽量保留原配置含注释), `--only-missing` 只导出未配置站点。
9. **状态持久化**: 规则执行历史/上传量快照/自动分类记录/曲线状态/跳检备份元数据 → `state_file` (JSON), 仅退出时落盘。
...
├── tests/                  # 29 个测试文件 + helpers.py (见 07)
