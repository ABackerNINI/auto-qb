...
| `torrents.py` | 667 | 种子数据层 | `TorrentRecord`(快照记录+惰性缓存 + `check_hr_condition/check_hr_satisfied` HR 判定), `TorrentStore`(refresh/分组索引/全局缓存/写后同步 + `restore_torrent` 跳检重加快照恢复 + `view_changed`/`consume_view_changed` 视图脏标记), **增量同步层 `TorrentSync`/`SyncTorrent`**(`/api/v2/sync/maindata` rid 合并: 只收变化种子的变化字段; Mapping+属性访问双通道视图; 端点不可用降级 `torrents_info`); `QbCompatError`/`missing_torrent_fields`/`REQUIRED_TORRENT_FIELDS`/`RE_ADD_FIELDS`/`_VIEW_FIELDS` |
