...
| `config/schema.py` | ~300 | **图形化配置编辑器的 UI 元数据单一来源**(纯声明, 无副作用): `Field`(key/label/kind/default/unit/options/help/required/placeholder/show_if/min/max/fields/optional/ui_only/**grey_if/group_of/risk**)、`Group`(左导航分组, 含 **icon**)、`Plugin`(插件 spec 形态: spec_kind/item_kind/options/placeholder/fields/**risk**); 内容: `GROUPS`(8 个分组, 顺序即表单顺序)/`TRACKER_FIELDS`(含站点 hr 子段)/`RULE_FIELDS`/`CONDITION_PLUGINS`(15)/`ACTION_PLUGINS`(12)/常量选项表 + `config_fields`/`real_config_fields`/`plugin_table`/`plugins_by_kind`/`schema_payload`; **不承载正确性规则**(合法性唯一入口仍是 validation), 键集合与插件表由 `tests/test_config_schema.py` 守卫 |
...
| `web_ui/static/` | — | 前端(Vue 3 CDN 本地托管, 无构建链) | index.html(根模板 + **图标 sprite**(`<svg class="icon-sprite">` 内 symbol + `<use href="#i-*">`, 无外部图标库依赖; 新增图标只改 sprite **同一处**) + `ce-field` 的 x-template + **toast 容器与站内确认框**单例 DOM)/app.js(顶栏搜索框在真实辅种组上筛选(任一成员命中即保留整组, 组行沿用真实组 key → 组级操作可用; 命中成员与组行均 `search-hit` 高亮; 未归组命中以单种子虚拟行 `u-<hash>` 兜底, 右键退化为单种子菜单) + **顶栏统计胶囊定宽槽**(数值右对齐, 速度变化不抖动) + **状态分布条与筛选区**(前端聚合 `members[].kind`: 堆叠条 + 可点击 chip 状态筛选 `kindFilter` + 保存路径筛选 `pathFilter` + 当前筛选/搜索说明, 三者叠加: 先筛状态/路径再匹配搜索) + **组级派生列**(computed `decoratedGroups` 一次性计算: 状态摘要图标与配色、共同标签/分类(交集/一致值, 差异显示 `±`/多分类)、保存路径、大小一致性) + 分组列表 **rid 增量轮询**(带 `?rid=` 请求, `updated:false` 时只更新徽章不重建数据) /展开明细(站点·状态·下载·上传·总上传·大小·标签·分类·进度·做种时长·Hash)/右键菜单(**视口边界吸附** `_menuPos`, Esc/外部点击关闭); **列宽记忆 = 稀疏 px 覆盖**(localStorage `autoqb_colwidths_v2`: 只冻结被拖动列的 px, 其余列保持 `minmax(...)+fr` 自动重分配; 双击列分隔线恢复默认; 列数变化须同步升 KEY 版本); **轮询策略**: `setTimeout` 链式续排(不堆叠请求) + 页面隐藏(`document.hidden`)停轮询 + 无变化退避 2s→5s→10s + 失败退避至 15s; **交互原语**: `toast(text, kind)` 替代 `alert`、`confirmDialog`/`promptDialog` 返回 Promise 替代 `confirm`/`prompt`(单例, 取消/遮罩/Esc 均结算为 false/null 且**不发出任何请求**)) /config_editor.js(图形化配置编辑器核心 mixin: 分组导航(带图标)/加载保存/只读预览/路径读写/列表与开关/字段灰显与风险标识/相关设置子卡/站点与曲线专段(含阶梯图)/`cfgFlatten` 扁平化渲染项; 删除类操作同样走 `confirmDialog`) /config_rules.js(规则集专段: 规则卡折叠与启用开关/条件与动作的选择面板与序号/15 条件/12 动作的插件与 spec 编辑, 动作按 trigger 过滤) /style.css(**单一 `:root` 设计令牌**: 背景/前景/语义色/表面/圆角/阴影/动效时长, 组件规则只引用变量(调色改一处); 组行/成员行状态色条与徽标、命中高亮、风险标识、子卡、选择面板、阶梯图等均在此; 响应式断点 1180px(隐藏指标胶囊)与 900px(顶栏折行 + 侧栏/主从单列))/vendor/vue.global.prod.js |
