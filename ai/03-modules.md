...
| `web.py` | ~230 | WEB UI 后端(FastAPI) | `create_app`(API: status/**state(支持 `?rid=` 版本门控: 版本一致时不回传 groups)**/groups/组控制/**config schema+树读取+树写回+只读预览**/**search 种子搜索**, 全部 Bearer 鉴权; 静态挂载 web_ui/static), `ensure_web_token`(随机生成持久化 data_dir/web.token), `start_web_server`(uvicorn 独立线程, stop 句柄); Web 线程只读快照+投递命令, 写操作经命令队列在主循环线程执行; 配置保存经 `config/writer.py`(校验/R 级回退/round-trip 写盘)后投递 `reload_config`; 搜索索引(hash->name+files)由主循环按需构建并原子替换, Web 线程只读, 种子名匹配即时(读 store.by_hash), 文件列表匹配依赖缓存索引(文件 API 只在主循环线程); 索引按 hash 增量维护(已建条目只刷新名称/新增补拉/已删淘汰) + 单次限流(SEARCH_INDEX_BUILD_BUDGET), 未拉完保持脏由下一 tick 续建, qB 断连时中止不写空 files |
| `web_ui/static/` | — | 前端(Vue 3 CDN 本地托管, 无构建链) | index.html(根模板 + `ce-field` 的 x-template)/app.js(顶栏搜索框在真实辅种组上筛选(任一成员命中即保留整组, 组行沿用真实组 key → 组级操作可用; 仅命中成员 search-hit 高亮; 未归组命中以单种子虚拟行 `u-<hash>` 兜底, 右键退化为单种子菜单; 非扁平"站点种子"列表) + 分组列表 **rid 增量轮询**(带 `?rid=` 请求, `updated:false` 时只更新徽章不重建数据) / 展开明细/右键菜单; **轮询策略**: `setTimeout` 链式续排(不堆叠请求) + 页面隐藏(`document.hidden`)停轮询 + 无变化退避 2s→5s→10s + 失败退避至 15s) /config_editor.js(图形化配置编辑器核心 mixin: 分组导航/加载保存/只读预览/路径读写/列表与开关/站点与曲线专段/`cfgFlatten` 扁平化渲染项) /config_rules.js(规则集专段: 规则级字段 + 15 条件/12 动作的插件与 spec 编辑, 动作按 trigger 过滤) /style.css(深色主题)/vendor/vue.global.prod.js |
| `web_ui/static/` | — | 前端(Vue 3 CDN 本地托管, 无构建链) | index.html/app.js(顶栏搜索框在真实辅种组上筛选(任一成员命中即保留整组, 组行沿用真实组 key → 组级操作可用; 仅命中成员 search-hit 高亮; 未归组命中以单种子虚拟行 `u-<hash>` 兜底, 右键退化为单种子菜单; 非扁平"站点种子"列表) + 分组列表 **rid 增量轮询**(带 `?rid=` 请求, `updated:false` 时只更新徽章不重建数据) / 展开明细/右键菜单/设置 YAML 编辑; **轮询策略**: `setTimeout` 链式续排(不堆叠请求) + 页面隐藏(`document.hidden`)停轮询 + 无变化退避 2s→5s→10s + 失败退避至 15s) /style.css(深色主题)/vendor/vue.global.prod.js |
