...
| `web.py` | ~250 | WEB UI 后端(FastAPI) | `create_app`(API: status(**含 `version` 透出给顶栏**)/**state(支持 `?rid=` 版本门控: 版本一致时不回传 groups)**/groups/组控制/**config schema+树读取+树写回+只读预览**/**search 种子搜索**, 全部 Bearer 鉴权; 静态挂载 web_ui/static), `ensure_web_token`(随机生成持久化 data_dir/web.token), `start_web_server`(uvicorn 独立线程, stop 句柄), `_app_version`(**函数内延迟导入** `__version__`: `__init__.py` 先导入 qbmanager 再赋值版本号, 顶层导入会循环导入); Web 线程只读快照+投递命令, 写操作经命令队列在主循环线程执行; 配置保存经 `config/writer.py`(校验/R 级回退/round-trip 写盘)后投递 `reload_config`; 搜索索引(hash->name+files)由主循环按需构建并原子替换, Web 线程只读, 种子名匹配即时(读 store.by_hash), 文件列表匹配依赖缓存索引(文件 API 只在主循环线程); 索引按 hash 增量维护(已建条目只刷新名称/新增补拉/已删淘汰) + 单次限流(SEARCH_INDEX_BUILD_BUDGET), 未拉完保持脏由下一 tick 续建, qB 断连时中止不写空 files |
| `web_ui/static/` | — | 前端(Vue 3 CDN 本地托管, 无构建链) | index.html(根模板 + **图标 sprite**(`<svg class="icon-sprite">` 内 symbol + `<use href="#i-*">`, 无外部图标库依赖; 新增图标扣 sprite **同一处**) + `ce-field` 的 x-template + **toast 容器与站内确认框**单例 DOM)/app.js(顶栏搜索框在真实辅种组上筛选(任一成员命中即保留整组, 组行沿用真实组 key → 组级操作可用; 仅命中成员 search-hit 高亮; 未归组命中以单种子虚拟行 `u-<hash>` 兜底, 右键退化为单种子菜单; 非扁平"站点种子"列表) + **状态分布条**(前端聚合 `members[].kind`: 堆叠条 + 可点击 chip 按状态筛选 `kindFilter`, 与搜索叠加: 先筛状态再匹配搜索) + 分组列表 **rid 增量轮询**(带 `?rid=` 请求, `updated:false` 时只更新徽章不重建数据) /展开明细(状态圆点 + 细进度条)/右键菜单(**视口边界吸附** `_menuPos`, Esc/外部点击关闭); **轮询策略**: `setTimeout` 链式续排(不堆叠请求) + 页面隐藏(`document.hidden`)停轮询 + 无变化退避 2s→5s→10s + 失败退避至 15s; **交互原语**: `toast(text, kind)` 替代 `alert`、`confirmDialog`/`promptDialog` 返回 Promise 替代 `confirm`/`prompt`(单例, 取消/遮罩/Esc 均结算为 false/null 且**不发出任何请求**)) /config_editor.js(图形化配置编辑器核心 mixin: 分组导航/加载保存/只读预览/路径读写/列表与开关/站点与曲线专段/`cfgFlatten` 扁平化渲染项; 删除类操作同样走 `confirmDialog`) /config_rules.js(规则集专段: 规则级字段 + 15 条件/12 动作的插件与 spec 编辑, 动作按 trigger 过滤) /style.css(**单一 `:root` 设计令牌**: 背景/前景/语义色/表面/圆角/阴影/动效时长, 组件规则只引用变量(调色改一处); 响应式断点 1180px(隐藏指标胶囊)与 900px(顶栏折行 + 侧栏/主从单列 + 表格横向滚动))/vendor/vue.global.prod.js |
