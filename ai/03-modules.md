...
| `config/schema.py` | ~300 | **图形化配置编辑器的 UI 元数据单一来源**(纯声明, 无副作用): `Field`(key/label/kind/default/unit/options/help/required/placeholder/show_if/min/max/fields/optional/ui_only/**grey_if/group_of/risk/unit_default**)、`Group`(左导航分组, 含 **icon**)、`Plugin`(插件 spec 形态: spec_kind/item_kind/options/placeholder/fields/**risk**); 内容: `GROUPS`(8 个分组, 顺序即表单顺序)/`TRACKER_FIELDS`(含站点 hr 子段)/`RULE_FIELDS`/`CONDITION_PLUGINS`(15)/`ACTION_PLUGINS`(12)/常量选项表(含 **`UNIT_KINDS`/`TIME_UNITS`/`SIZE_UNITS`/`SPEED_UNITS`**) + `config_fields`/`real_config_fields`/`plugin_table`/`plugins_by_kind`/`schema_payload`; **不承载正确性规则**(合法性唯一入口仍是 validation), 键集合与插件表由 `tests/test_config_schema.py` 守卫 |
| `config/writer.py` | ~215 | 配置写回(图形化编辑器专用): `read_tree`(`BaseLoader` 读为 YAML 同构树, 标量全字符串)/`write_tree`(结构自检 → 临时文件 `load_config` 校验 → `diff_config_impacts` → **R 级字段回退磁盘旧值** → 备份 `.bak` → ruamel round-trip 写盘)/`preview_tree`(同前置逻辑, 不落盘, 供只读预览)/`_sync_mapping`(递归同步 CommentedMap: 保注释, 值未变则跳过赋值以保留原标量形态)/`_plain_scalar`(数字/布尔样式写为原生标量, 与磁盘既有书写风格一致)/`_same_value`+`_as_builtin`(按 BaseLoader 语义比较, 布尔小写化)/`WriteResult` |
...
| `qbmanager.py` | ~800 | 主协调者 | `QbManager`(6 mixin 组合): `run`(每轮经模块级 `_throttle`: 托管 stop_event -> `Event.wait`, 非托管 -> `time.sleep`)/`_tick`(refresh + `task_queue.run_due` 一次调用)/`_refresh_torrents`(走 `store.apply_sync` 增量)/`connect`(连接异常日志节流 `_last_conn_ok` 状态机; 客户端经 `_new_client` 构造 —— 本地地址用 `LocalQbClient` 关闭 requests `trust_env`)/`_create_global_tasks`/`_create_torrent_tasks`/`_handle_maintenance`; `_build_group_view`(分组视图快照; 子方法 `_hr_view_fields` 透出 HR 展示字段(标签文本 + hr_triggered/hr_satisfied/hr_req_time/hr_req_ratio), 组级另计 hr_triggered/hr_pending/added_on) |
...
| `torrents.py` | ~720 | 种子数据层 | `TorrentRecord`(**唯一数据所有者**: 21 快照 slot + `_raw` 非快照字段兜底 + 惰性缓存 + HR 判定; `apply_delta(patch)->变化字段集` 只遍历 patch 字段), `TorrentStore`(`apply_sync`/`reset_sync` = 增量同步态; `refresh` 全量入口; `_apply` 应用与变化集收集; 分组索引/全局缓存/写后同步 + `view_changed`/`consume_view_changed` + `delta_fields`/`state_changed`/`dirty_groups`/`rounds_applied` + `remove_torrent`/`restore_torrent` 待报删除); `QbCompatError`/`missing_torrent_fields`(支持映射与属性两类来源)/`REQUIRED_TORRENT_FIELDS`/`RE_ADD_FIELDS`/`_VIEW_FIELDS`(含 `tags`/`category` —— 组级标签/分类列的数据源) |
