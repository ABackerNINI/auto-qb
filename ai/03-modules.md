...
| `qbmanager.py` | ~800 | 主协调者 | `QbManager`(6 mixin 组合): `run`/`_tick`(refresh + `task_queue.run_due` 一次调用)/`_refresh_torrents`(走 `store.apply_sync` 增量)/`connect`(连接异常日志节流 `_last_conn_ok` 状态机; 本地地址额外 `_disable_env_lookup_for_local` 关闭 requests `trust_env`)/`_create_global_tasks`/`_create_torrent_tasks`/`_handle_maintenance` |
