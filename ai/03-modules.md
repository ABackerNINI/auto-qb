...
| `grouping.py` | 313 | 辅种分组 (事件驱动) | `_assign_new_torrent`/`_assign_to_group`/`_leave_group`(O(1) 成员索引), `_check_size_consistency`, `_handle_removed_torrents`/`_handle_state_transitions`/`_handle_save_path_changes`(→`_check_missing_files` 磁盘扫描), `_check_download_conflicts`(每轮), `_group_members`/`_group_has_downloading`/`_group_reference_candidates`(已完成且未校验, 供 checking 动作) |
