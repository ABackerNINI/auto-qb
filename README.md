...
- **辅种管理页**：辅种分组表格（组内速度 / 总上传 / 总大小 / 站点徽章，列头排序、拖拽调宽、展开各站点成员明细；数值列右对齐、零值居中显示）+ 按种子名 / 文件名搜索；多选（Ctrl/⌘ 点选、Shift 范围选择，展开的组可作为 Shift 起点）+ 批量操作浮条（开始 / 暂停 / 强制汇报 / 删除，选中后吸顶常驻）；组级与单种子级右键操作（暂停 / 开始、强制汇报、删除），命令带执行结果回执（强制汇报有 等待中 / 汇报成功 / 超时失败 三态强反馈，tracker 确认窗口 30s）；删除确认框加宽展示待删目标信息（整组逐个列出成员种子）并提供"删除前强制汇报"（默认勾选，汇报失败则不删除）与"删除磁盘文件"选项；今日流量面板可打开历史流量折线图（天 / 月 / 年视图，悬停取值）
- **图形化设置页**：全部配置表单化增删改——基础设置、站点、规则集（15 种条件 / 12 种动作的选择、排序与参数编辑）、全局限速曲线等（日志 / WEB UI / 通知短段平铺不折叠）；只读 YAML 预览，保存走启动同路径校验 + `.bak` 备份 + 热重载（需重启项自动回退并提示）
...
**两套界面（随时互切）**：根路径 `http://127.0.0.1:8080/` 默认进入**星图**（经典深色仪表盘，单主题高信息密度）；浏览器访问 `http://127.0.0.1:8080/prism/` 使用**棱镜**——工程仪器风格的全新视觉与多主题，顶栏「主题」按钮可随时切换 5 套主题：深海机房 / 暗夜星云 / 极地晨霜 / 麦秋 / 品牌轨道（暗色系统默认「深海机房」，浅色系统默认「极地晨霜」，手动选择后保存在浏览器本地）。两套界面顶栏随时互切。目录各自落位：`web_ui/static/atlas/`（星图）与 `web_ui/static/prism/`（棱镜，独立 HTML/CSS 令牌分层 + 主题引擎，不引构建链），公共前端逻辑层单一来源收在 `web_ui/static/shared/`，与 `/api/*` 共享，功能等价；升级前的 `/newui/` 旧链接自动重定向到 `/prism/`。新增界面 = 在 `static/` 下加同名目录（命名约定：英文小写目录名 + 两字中文名，取身份名不取代际名）。
