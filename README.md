...
- **可视可控** — 内置 Web UI（默认关闭）：辅种分组 / 单种子双视图、种子搜索、右键控制 + 全部配置图形化增删改（自动校验、备份、改完即生效）；另支持 `--tray` 托盘常驻与系统原生桌面通知
...
- **图形化设置页**：全部配置表单化增删改——基础设置、站点、规则集（15 种条件 / 12 种动作的选择、排序与参数编辑）、全局限速曲线等；只读 YAML 预览，保存时自动校验 + `.bak` 备份 + 改完即生效（需重启的项自动回退并提示）
...
- **全部配置表单化增删改**：基础设置、qB 连接、日志、通知、站点（左列表 + 右详情）、规则集（规则 + 15 种条件 / 12 种动作的选择、排序与参数编辑）、全局限速曲线等
