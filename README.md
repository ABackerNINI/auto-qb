...
- 做种时间满足「要求时间 + 额外时间」后自动添加完成标签或分类,如 `--HR3D--`
- 站点配置可覆盖全局配置
        ...
        rule1:
            ...
            actions:           # 动作顺序执行,默认一个出错后续不执行
                ...
                - move_to:                        # 移动保存路径
                    ...
                    overwrite: true
                - reannounce: true                # 强制汇报 tracker(有风险)
...
- **无参考种子**:`full-checking` __<font color="green">安全</font>__;`skip-checking` 为 __<font color="red">高风险</font>__ (仅做基础文件存在与大小对比,不做哈希校验直接开始,文件内容错误时会传垃圾数据,被大部分PT站点严令禁止,见[风险与约束](#风险与约束))!
...
- 内置步骤(缺文件检查 → tracker 标签 → HR)只在种子未匹配任何启用的规则时兜底执行
- 规则动作执行后内置步骤跳过,避免冲突(如规则 `start` 了种子,内置缺文件检查又 `stop` 它)
- 缺文件检查优先级最高,`MISSING` 的种子任何规则都不生效,直到文件恢复并 recheck 通过
...
- 新种子添加时自动归组(不再每轮全量遍历);文件大小一致性仅在归组时检查
- 缺文件检查事件驱动(组内删除/转暂停/路径变化时立即触发,无周期轮询),文件丢失 → 整组暂停 + `MISSING` 标签
- 组内两个及以上种子同时下载、或已完成与下载中并存 → 暂停并警告
...
├── exporter.py        # YAML 配置模板导出(只追加,保留注释)
├── episodes.py        # 集数标签解析
...
```
├── src/auto_qb/    # 主代码
├── tests/          # 测试(test_*.py,helpers.py)
├── config.yml      # 配置文件
├── 想法.md         # 设计文档(功能设计/实现细节/风险与约束)
└── env.sh          # 依赖安装脚本
```
...
- "下载量/完成量/进度为 0 + 暂停"也可能命中用户"先加种子稍后下载"的正常操作,必须限制在显式允许的 tracker/分类内
- 跳检前只做"文件存在 + 大小一致"检查,内容错误时会把垃圾数据标为已完成并上传 → 可能被站点处罚
- 当前"导出 → 删除 → 重加"的实现有中断窗口(崩溃即丢种子),且本地统计(`seeding_time`/`uploaded`)清零,影响 HR-DONE 判定和上传量规则
- 约束:文件检查不通过时降级为 `torrents_recheck`

### 强制汇报(reannounce)

- 只能在状态首次变化(如刚置为 HR-DONE)时汇报一次;高频 announce 可能被站点视为异常行为

### 限速覆盖

- 无条件按配置设置限速会覆盖用户手动修改的值
...
- 多 tracker 种子:标签取所有匹配 tracker 的并集,限速冲突取更严格值并打日志

### MISSING 恢复

- 暂停并打 `MISSING` 标签后,文件恢复不会自动解除
- 计划:对 MISSING 种子每轮重查文件 → 齐了则 recheck → 通过后移除标签并恢复原状态

### 大库性能

- 每轮避免对每个种子重复调用 `torrents_trackers` / `torrents_files`(缺文件检查与辅种检查合并为一次文件列表获取)
- 文件级检查降频(如每 5 轮一次全量缺文件扫描)
