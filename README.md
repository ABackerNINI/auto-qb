...
- **辅种管理页**：辅种分组表格（组内速度 / 总上传 / 总大小 / 站点徽章，列头排序、拖拽调宽、展开各站点成员明细；数值列右对齐、零值居中显示）+ 按种子名 / 文件名搜索；多选（Ctrl/⌘ 点选、Shift 范围选择）+ 批量操作浮条（开始 / 暂停 / 强制汇报 / 删除）；组级与单种子级右键操作（暂停 / 开始、强制汇报、删除），命令带执行结果回执（强制汇报等待 tracker 确认后才提示成功，超时 30s）；删除确认框展示待删目标信息并提供"删除前强制汇报"（默认勾选，汇报失败则不删除）与"删除磁盘文件"选项；今日流量面板可打开历史流量柱状图（天 / 月 / 年视图，悬停取值）
