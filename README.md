...
**新版界面（可选切换）**：浏览器访问 `http://127.0.0.1:8080/newui/`（端口以配置为准）体验全新视觉与多主题。顶栏「主题」按钮可随时切换 5 套主题：深海机房 / 暗夜星云 / 极地晨霜 / 麦秋 / 品牌轨道（暗色系统默认「深海机房」，浅色系统默认「极地晨霜」，手动选择后保存在浏览器本地）。旧版界面原样保留，两版顶栏随时互切。新版为独立目录 `web_ui/static/newui/`（独立 HTML/CSS 令牌分层 + 主题引擎，不引构建链），与旧版共享同一套前端逻辑与 `/api/*`，功能等价。
