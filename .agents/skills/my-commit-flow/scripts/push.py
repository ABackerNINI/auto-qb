...
def main(argv: list[str] | None = None) -> int:
    ...
    git("fetch", REMOTE_MAIN, BRANCH)
    behind_ahead = git("rev-list", "--left-right", "--count", f"{REMOTE_MAIN}/{BRANCH}...HEAD").stdout.split()
    ...
    print(f"\n=== 推主线 {REMOTE_MAIN}/{BRANCH} ===")
    proc = git("push", REMOTE_MAIN, BRANCH)
    ...
    print("\n=== 核对远端 ===")
    remote = git("ls-remote", REMOTE_MAIN, BRANCH).stdout.split()
