...
| 步 | 做什么 | 判据 / 不通过怎么办 |
|---|---|---|
| **S0 认远端** | `python scripts/preflight.py` | 主线远端必须指向 `gitee.com`；上游必须是 `<main>/<branch>`。`git pull <remote>` 不带分支名**只 fetch 不合并**，分支名必须写 |
...
| **S3 暂存** | `scripts/commit.py --message-file <文件> <路径...>` | **逐路径**，脚本直接拒绝 `-A` / `.` / `*`；暂存清单里出现 `config.yml` / `auto-qb-data/` → 脚本拒提交；`想法.md` → WARN，确认是用户自己的改动才可入 |
...
| **S6 推送** | `python scripts/push.py` | **顺序固定**：先 Gitee（必须成功）→ 核对远端 ref == 本地 → 再尝试一次 GitHub 直连（禁代理 `-c`）。直连失败**只报一次**：不重试、不换代理、不改 SSH、不回滚 Gitee |
...
| 脚本 | 职责 | 退出码 |
|---|---|---|
| `scripts/preflight.py` | 只读预检（+ 一次安全 fetch）：远端 / 上游 / 落后 / 脏 / 红线 / 闸门 / staged 异常 | 0 可继续 · 1 有 STOP |
| `scripts/commit.py` | 逐路径 `add` + `commit -F` + 提交后自动核 ref | 4 参数/红线 · 5 git 失败 · 2 ref 不一致 |
| `scripts/verify_ref.py [sha]` | ref 三处一致核对 | 0 一致 · 2 不一致 · 3 staged 暴增 |
| `scripts/push.py [--skip-mirror]` | fetch → 推主线 → 核对远端 → 尝试一次镜像 | 0 主线成功 · 1 落后 · 5 主线失败 |

配置单点在 `scripts/_ship_config.py`（远端名 / 分支 / 红线清单 / 闸门映射 / 镜像参数）。
