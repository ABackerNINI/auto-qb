---
name: aesthetic-preset-library
description: 20 种设计哲学风格库（5 流派 × 4 设计师），用于在缺乏视觉参考时为设计任务做审美锚定。每种风格含哲学内核 / 核心特征 / 提示词 DNA / 代表作 / 搜索关键词。覆盖网页 / App / PPT / PDF / 信息图 / 封面 / AI 生成等场景。当用户没有提供截图、参考产品或设计系统时调用本 skill 推荐 2-3 个跨流派 preset 让用户选择。
---

# 设计哲学风格库：20 种体系

> 用于视觉设计（网页/PPT/PDF/信息图/配图/App 等）的设计风格库。
> 每种风格提供：哲学内核 + 核心特征 + 提示词 DNA（与场景模板组合使用）。
...
> 场景适配：★★★ = 强烈推荐 / ★★☆ = 适合 / ★☆☆ = 需改造
> AI生成：★★★ = 直出效果好 / ★★☆ = 需调整 / ★☆☆ = 建议 HTML 执行
> 最佳路径：AI 生成（图片直出）/ HTML（代码渲染，数据精确）/ 混合（HTML 布局 + AI 配图）

**核心规律**：有明确视觉元素的风格（插画/粒子/生成艺术）AI 直出效果好；依赖精确排版和数据的风格（网格/信息架构/留白）HTML 渲染更可控。
...
### 01. Pentagram - Michael Bierut 风格
**哲学**：字体即语言，网格即思想
**核心特征**：
- 极度克制的颜色（黑白+1 个品牌色）
- 瑞士网格系统的现代演绎
...
**代表作**：Bill & Melinda Gates Foundation 年度报告
...
### 10. Müller-Brockmann 传承 - 瑞士网格纯粹主义
**哲学**：客观性即美
**核心特征**：
- 数学精确的网格系统（8pt 基线）
- 绝对的左对齐或居中
...
根据速查表的「最佳路径」列选择：
- **AI 生成**：有明确视觉元素的风格（06/07/12/13/14/15/16/20），用 Gemini/Midjourney 直出
- **HTML 渲染**：依赖精确排版的风格（01/03/04/10/11/17/18），代码控制数据和布局
- **混合**：HTML 做骨架布局 + AI 生成配图/背景（02/05/08/09/19）

### 质量控制

1. ❌ 不要直接写 "in the style of Pentagram" → ✅ 用具体设计特征描述
2. 文字在 AI 生成中常出错 → 生成后替换文字
3. 比例易失真 → 明确指定 aspect ratio
4. 先生成 3-5 个变体，选择最佳后细化
...
被 APP_WORKFLOW / WEB_WORKFLOW 在「视觉锚定 fallback」环节调用时：
