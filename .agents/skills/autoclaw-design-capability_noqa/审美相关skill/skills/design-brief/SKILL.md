...
The values listed below form a closed vocabulary. Only values in this table have concrete token mappings in Section 2.1. If the user provides a value not listed here, map it to the closest supported semantic value; if no close match exists, use the Section 2.2 default and record the substitution.
...
Symbolic values not in this table are not valid. For an unrecognized value such as `palette=ocean_blue`, choose the closest supported semantic value when defensible; otherwise use the corresponding Section 2.2 default and record the substitution as an adjustable assumption.
...
This skill generates a new DESIGN.md from scratch based on the resolved brief dimensions. If a DESIGN.md already exists, preserve it and write a versioned candidate such as `DESIGN.v2.md` unless the current task explicitly authorizes replacing the existing file.
