...
### 0.C If the brief is ambiguous, choose a coherent direction
When plausible design reads genuinely diverge, compare them internally and choose the direction that best matches the audience, task goal, references, and quiet constraints. State the selected design read, record the assumption as adjustable, and proceed without a clarification stage.
