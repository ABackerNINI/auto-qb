...
- If the intent is ambiguous, use a live artifact when the brief contains data freshness, provenance, connector, dashboard, or recurring-update signals; otherwise choose a normal static artifact and record the assumption.
...
1. **Resolve scope and data source without blocking on connected connectors**
   ...
   - When several connected candidates fit, choose the explicitly named source first, then the closest semantic match. If no matching connector or usable topic/query exists, explain the concrete data-source blocker instead of inventing data or opening a clarification stage.
