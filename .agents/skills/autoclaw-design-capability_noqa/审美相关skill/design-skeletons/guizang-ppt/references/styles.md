...
5 个**预设方向**，每个方向都把"用哪套主题色 / 哪些 layout / 多少 slide / 怎么写 chrome 文案"打包好，避免方向决策时临场混搭。
...
在 SKILL.md `Step 0 · 定方向`：根据用户主题、受众、素材和语气，从这 5 个方向里自动选一个。流程是：

```
1. 用户讲一句"想做个分享 PPT"
2. 你（agent）按推荐速查判断方向
3. 方向决定主题色、slide 数量区间和 chrome 风格
4. 其余字段按 brief / 素材 / 默认策略补齐
```

**硬规则**：方向只能从下面 5 个里选，不能混搭。混搭 = 走 huashu-design 验证过的失败路径(品牌资产协议 v1)。如果用户语义不贴合任何方向，选最接近的，然后允许在 `chrome` / `kicker` 里轻微定制语气，**绝不调色**。
...
定完方向后，**在项目目录下生成或更新 `项目记录.md`**（或 `大纲-v1.md`），第一行写清：
...
- ✅ 只在 5 个方向里挑，挑完用方向补齐主题色、slide 数量和 chrome 风格
