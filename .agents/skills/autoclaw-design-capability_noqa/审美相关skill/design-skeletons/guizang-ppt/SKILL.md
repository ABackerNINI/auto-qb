...
od:
  ...
  design_system:
    requires: false
  example_prompt: "帮我做一份杂志风的 PPT —— 关于'一人公司 · 被 AI 折叠的组织'，25 分钟分享会，目标受众是设计师 + 创业者。"
---
...
This skeleton contains structural Chinese typography (bilingual font stacks, zh-CN CSS classes, CJK navigation hints) that is part of its design identity. When instantiating for a user, the agent MUST translate ALL placeholder visible copy (headings, labels, body text) to the user's conversation language, but MUST preserve structural/typographic CJK elements (font stacks, CSS class names, bilingual layout hints) that serve the design's bilingual capability. The `<html lang>` attribute must match the user's language.
...
### Step 0 · 定方向(Direction · 必做的第一步)

Phase 1 起手先根据主题、受众、素材类型和语气，在 5 个 magazine 方向里选定一个最合适的方向，然后继续 Step 1 / Step 2。

每个方向都已经打包了"主题色 / 推荐 layout / chrome 风格 / 推荐 slide 数"。不要混搭方向；不确定时选默认方向。

打开 `references/styles.md`，按用户主题自动选择 1 个方向：
...
默认选 Monocle Editorial，因为它失败概率最低。如果用户提到"AI / benchmark / 技术发布"——选 WIRED；"读书 / 私享 / 朋友圈"——选 Kinfolk；"设计 / 建筑 / portfolio"——选 Domus；"研究 / 学术 / 方法论"——选 Lab。

定完方向后，在项目目录下创建或更新 `项目记录.md`，第一行写清方向 + 主题色 + 受众 + 时长(模板见 `styles.md` 末尾)。**全程不要换方向**——半路换 = 前面全废。

### Step 1 · 需求补齐(**动手前必做**)

完整大纲和图片优先作为输入使用；缺少的信息按下面清单自动补齐，写入 `项目记录.md` 或 `大纲-v1.md`。

**如果用户只给了主题或一个模糊想法**，按这 6 项做内部补齐后再动手。不要无记录地开始写 slide——一旦结构定错，后期翻修代价很高:

#### 6 项补齐清单

> 第 5 项已在 Step 0 选方向时一并补齐(方向→主题色)。下面清单中第 5 项留白即可。

| # | 字段 | 默认决策 |
|---|------|-----------|
| 1 | **受众 / 分享场景** | 未指定时按“产品与业务团队内部分享” |
| 2 | **分享时长 / 页数** | 主题 brief 默认 8-10 页；资料充分按方向建议页数 |
| 3 | **原始素材** | 有文档 / 数据 / 旧 PPT / 文章链接则优先采用；没有则搭建可信结构与占位 |
| 4 | **图片使用** | 有图片则进入 `images/` 约定；没有则用 SVG / CSS 图形与留白结构替代 |
| 5 | **主题色** | 已在 Step 0 由方向决定 |
| 6 | **硬约束** | 未指定时只遵守通用版权、安全、真实性与字号约束 |
...
如需要图片素材，按以下约定组织；最终交付说明里简短提示替换方式：
...
3. Step 1 需求补齐完成后，读 `themes.md` 看色板细节
