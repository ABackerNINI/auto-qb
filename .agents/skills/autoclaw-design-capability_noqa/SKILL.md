...
For a standard new Phase 1 task, infer missing preferences from the user query, attachments, PRD/QA materials, code baseline, selected skeleton defaults, design-system fit, and `TASK_ROUTER.md` fallback rules. Continue directly into `INDEX.md`, the three-axis route, and generation without a clarification stage. Read `INTERACTIONS.md` only for status, plan, preview, and other non-question front-end actions.
