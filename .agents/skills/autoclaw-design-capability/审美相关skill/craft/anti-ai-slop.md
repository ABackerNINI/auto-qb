...
2. **Two-stop "trust" gradient on the hero** — purple→blue, blue→cyan,
   ...
   time.
3. **Emoji as UI elements** — any emoji character used as icon,
   avatar placeholder, product image, navigation element, or
   decorative visual in the rendered HTML. Use
   1.6–1.8px-stroke monoline SVG with `currentColor`. The only
   acceptable emoji context is inline body copy where the emoji
   IS the content (e.g., a chat message containing emoji).
