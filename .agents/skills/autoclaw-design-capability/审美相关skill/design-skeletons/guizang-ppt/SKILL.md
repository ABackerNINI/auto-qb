...
AutoDesign 场景下，Phase 1 opening message 必须先输出 `INTERACTIONS.md §1` 的 `<question-form>`。不要用 markdown 散文列 5 个方向，也不要先进入 Step 1/Step 2。

把 5 个 magazine 方向放进同一张 `question-form` 的 `visualStyle` 题里，并在同一张表单内一次问完受众、时长、素材、图片、硬约束、讲稿深度和补充说明。每个方向都把"主题色 / 推荐 layout / chrome 风格 / 推荐 slide 数"打包好，用户提交/跳过/超时后再继续。
...
**不能因为用户已经给了完整的大纲 + 图片而跳过 Phase 1 `<question-form>`**。完整大纲和图片只表示表单里很多项可以默认选“你来判断”或预填，不表示可以直接进 Step 2。
