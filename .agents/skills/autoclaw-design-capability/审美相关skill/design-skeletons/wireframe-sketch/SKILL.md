...
When instantiating this wireframe skeleton for a user, the agent MUST write ALL visible copy (tab labels, annotation text, placeholder labels) in the user's conversation language. Hand-drawn wireframe aesthetic (graph-paper, marker strokes, hatching) stays unchanged. The `<html lang>` attribute must match the user's language.
