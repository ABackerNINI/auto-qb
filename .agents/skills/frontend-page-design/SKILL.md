...
description: Design and build polished frontend pages for websites, landing pages, product introductions, portfolios, and other presentation-focused web experiences. Use when generating or redesigning visually distinctive HTML, CSS, and JavaScript pages with responsive layouts, real imagery, restrained motion, and anti-template quality checks. Do not use for reports, data analysis, charts, dashboards, admin tables, or research reports; route those tasks to delivery-artifact or design-capability.
---
...
## ⚡ SINGLE-FILE NATIVE MOTION OVERRIDE (highest priority in this deployment, read first)

This skill ships **HTML pages** inside AutoClaw. Therefore every React / Next.js / framer-motion / `motion/react` / `useMotionValue` implementation note below **does not apply** — their **design intent (when to move, how much, why) is fully preserved**, but the implementation switches to the native stack below. Hard constraints: **plain HTML/CSS/JS, no npm, no build step, no external motion libraries or CDNs (hand-write native motion). Images keep taste-skill's original approach (image hosts are allowed).**

**Motion dial: restraint first. `MOTION_INTENSITY` defaults to 4-5, and never above 5 for showcase pages.** Motion does only three things: scroll-in reveals on key sections (light), a persistent micro-motion on the hero (very light, e.g. slow background drift), and hover feedback on CTAs/cards. **Forbidden:** marquees, gradient orbs / glow blobs as filler decoration, infinite-loop animations everywhere, purely decorative motion. taste-skill's own Rule 39 ("don't put infinite micro-animations everywhere") is law. **Motion is the supporting act — content and imagery are the lead. A page must stand on real photography and typography, not on motion.**

Images keep taste-skill's original strategy (put real photography in image slots; never use gradient blocks / abstract SVG shapes / emoji to fill a slot that should hold a photo — that is the #1 AI-slop tell).

### Native motion recipes (copy directly, all zero-library)

- **Persistent micro-motion (hero gradient drift / float):** pure CSS `@keyframes` + `animation ... infinite alternate`; give the hero background a slow translate/scale so the first screen feels alive.
- **Headline character/word reveal:** split the headline into `<span>`s, each with `animation-delay: calc(var(--i)*.08s)`, `@keyframes` from `opacity:0;translateY(40px)` to 0.
- **Scroll-in reveal:** prefer `animation-timeline: view()` (native scroll-driven, 2024+), wrapped in `@supports (animation-timeline: view())`; add an `IntersectionObserver` `.seen` fallback so older browsers still work. Layer both.
- **Scroll scrub / parallax:** `animation-timeline: scroll(root)` so elements shift with scroll progress (horizontal-drift headlines, layered parallax).
- **Magnetic button / 3D tilt:** read pointer offset on `mousemove` → write `el.style.transform` directly (`translate` or `rotateX/Y`), reset on `mouseleave`. **Do not** track continuous pointer values through a state framework; writing transform directly is the lightest path.
- **Count-up numbers:** `requestAnimationFrame` interpolate 0→target, triggered when the element enters the viewport.
- **Infinite marquee:** duplicate the content twice + `@keyframes translateX(0→-50%)` `linear infinite`; at most one per page.

### Motion discipline (hard rules, self-check each before delivery)

1. **Content must stay visible under reduced-motion:** inside `@media (prefers-reduced-motion: reduce)` turn off `animation`/`transition`, but **force every initial hidden state (`opacity:0` etc.) back to visible** — never allow "motion off = content gone forever."
2. **First screen never renders in a hidden state:** on static open (JS not run / animation not triggered) the first screen must show content; no full-screen blank waiting for a reveal.
3. **No emoji filler when images are absent:** use inline SVG or characters for icons; never use emoji as imagery/icons.
4. **Mobile degrade:** heavy motion (parallax, tilt) falls back to static on narrow screens or under reduced-motion.
