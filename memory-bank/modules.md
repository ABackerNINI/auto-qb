...
| `torrents/` (包) | 766/5 文件 | 种子数据层(2026-09-15 由单文件 725 行转包): `compat.py`(qB 字段契约: `_SNAPSHOT_FIELDS`/`RE_ADD_FIELDS`/`REQUIRED_TORRENT_FIELDS`/`QbCompatError`/`missing_torrent_fields`)/`view.py`(Web 视图字段契约: `_VIEW_FIELDS`/`_VIEW_QUANTUM`/`view_field_value`/`view_dirty`)/`record.py`(`TorrentRecord`)/`store.py`(`TorrentStore` + `_SYNC_UNAVAILABLE` 降级信号)/`__init__.py`(全量重导出, 调用方零改动) |
...
| `web_view.py` | 279 | WEB 视图组装(2026-09-15 拆分) | `WebviewMixin`: `_build_group_view`(分组视图)/`_build_singles_view`/`_member_view`(单种子视图, 第八轮)/`_hr_view_fields`/`_state_kind`(状态语义分类)/`ensure_group_view`/`ensure_group_state`(rid 版本门控)/`_build_search_index`(增量限流)/`search_torrents`/`touch_web_client`; 常量 `SEARCH_INDEX_BUILD_BUDGET`; 依赖宿主 store/client/web_commands/视图状态属性 |
