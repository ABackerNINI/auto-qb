...
| `torrents/` (包) | 808/5 文件 | 种子数据层(2026-09-15 由单文件 725 行转包): `compat.py`(qB 字段契约: `_SNAPSHOT_FIELDS`(全量 70 字段: 必需 21 + 重加升格 6 + 可选扩展 43, 2026-09-15 全量扩容, 字段定稿参照真机样例)/`RE_ADD_FIELDS`/`REQUIRED_TORRENT_FIELDS`(校验面不变)/`QbCompatError`/`missing_torrent_fields`)/`view.py`(Web 视图字段契约: `_VIEW_FIELDS`/`_VIEW_QUANTUM`/`view_field_value`/`view_dirty`)/`record.py`(`TorrentRecord`: slots 全量缓存 qB 种子数据字段(为 WebUI 后续功能供数), 可选字段缺失保持 qB 哨兵默认(-1/-2/8640000), `_raw` 仅收未声明字段(前向兼容), `to_dict()` 全字段导出)/`store.py`(`TorrentStore` + `_SYNC_UNAVAILABLE` 降级信号)/`__init__.py`(全量重导出, 调用方零改动) |
...
| `web_view.py` | ~500 | WEB 视图组装(2026-09-15 拆分; 同日追剧视图扩容) | `WebviewMixin`: `_build_group_view`(分组视图)/`_build_singles_view`/`_member_view`(单种子视图, R08)/`_build_shows_view`(追剧视图, R10: 全量 by_hash 按 (剧键, 季, 集键) 聚合, 集级聚合后端算好, members 只放 hash 由前端从 groups∪singles 成员索引取; 文件兑底 pending→`_build_search_index` 置脏重建闭环)/`_hr_view_fields`/`_state_kind`/`ensure_group_view`(三视图同脏窗口同快照重建)/`ensure_group_state`(rid 版本门控, state 含 groups+singles+shows)/`_build_search_index`(增量限流, 新增条目时经 `_trigger_shows_rebuild_if_pending` 触发追剧视图重建)/`search_torrents`/`touch_web_client`; 常量 `SEARCH_INDEX_BUILD_BUDGET`; 依赖宿主 store/client/web_commands/视图状态属性 |
