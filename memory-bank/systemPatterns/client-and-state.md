...
- `_load_state()`: JSON 读入 `self.state` (构造时 + run 时 + 热重载 L2 各一次); `save_state()`: 优雅退出(run 的 finally)+ 主循环周期调用 (`_maybe_flush_state`, 间隔 `state_save_interval` 默认 120s / 配置端下限 30s / 0=关闭 —— 2026-09-22 起, issue 26-09-21-1347 推翻「仅退出落盘」的旧取舍)。**写点即时落盘的例外**: `skip_check_day`(跳检完成标记)与 `recheck_fails`(失败冷却)在动作写点直接 save —— 重放代价最高; 其余键靠周期兑底, 新增写点无需插桩。
