...
- **schema 版本迁移 (2026-09-26, 计划 26-09-26-0506)**: 落盘结构化文件(state.json / hr/<site>.json / config.yml)统一带 `schema_version`, 框架单点 `infra/versioning.py` —— `CURRENT_VERSIONS[kind]` 当前版本 + `MIGRATIONS[kind][from]` 相邻版本迁移表(每项恰好 +1 级, 纯函数无 IO, 跨多级由 `migrate()` 沿链串行)。**口径**: 字段缺失 = v1(存量文件零迁移成本); 版本落后 → 加载时沿链迁移(「旧迁」); 比程序新 → fail-fast(`SchemaVersionError`, **不算 `_CORRUPT`** —— 版本问题不是内容损坏, 混进三态会触发错误的 .bak 回退; 备份与主文件同版本, 回退无意义)。落盘时机按 kind 各自的物化点: state 在 run() 持锁后 `_materialize_state_migration`(仅磁盘版本落后时写一次, dry-run 只内存)、hr 随下次 `HrLockSession.commit`、config 随下次 WebUI 保存(writer/`--export-yaml` 盖章, 运行期不主动写回)。刻意不为「补字段」类非破坏性动作造版本 —— 链式执行的正确性由单测临时注册验证, 不落生产。
