...
- **设置页刷新保持位置 (2026-09-25, ✅ 已修并双验 → 剩真机走查)**: 用户报"设置页刷新会回到种子页"。
  修法: 顶层 `page`(`autoqb.ui.page`)与设置分区 `hub.view`(`autoqb.ui.hub`)落盘 + 启动补一次 `cfgLoad`。
  **真机走查**: ①进设置页点开任一分区(如「站点」), 按 **F5** —— 应仍停在该分区(不是设置首页、更不是种子页);
  ②关掉浏览器重开该地址 —— 应仍停在设置页原分区(位置与 `autoqb.ui.view` 同族, 是**跨会话**偏好);
  ③顶栏点「分组/种子/追剧」应正常回辅种页(切页即落盘); ④⚠ 若你用的是浏览器"关闭窗口时清除 Cookie 和站点数据"
  的例外, 重开后回种子页属**浏览器侧清数据**, 不是本 bug 复发(见 [pitfalls/web-ui/columns-persist.md](../pitfalls/web-ui/columns-persist.md))。
  ⚠ 已知**未做**(已入池, 见 `issues/26-09-25-1702-bug-webui-settings-unsaved-changes-lost.html`):
  刷新会丢未保存的配置改动(`cfg.tree` 是内存态, 脏时刷新无 `beforeunload` 提醒) —— 与本条是两个问题。
...
- **时间点列表头右键切换相对/绝对 (FX-28, 2026-09-20, ✅ 已提交并推送 `7548e51`; 剩真机目视)**: 覆盖**四个时间点列**
  ...
  原为绝对, 最近活动已改相对) —— 加开关不顺手改观感。**时长列(做种时长/活跃时间/剩余时间[原 ETA, 列 key 仍 eta])不参与**: 它们不是时间点,
