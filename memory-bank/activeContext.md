...
- **种子页「可用性」列隐藏负数与暂停值 (2026-09-20, ✅ 已实施并冒烟验证; 未提交; 剩真机目视)**: 用户报该列出现负数
...
- **种子页「最近活动」列改相对时间 (2026-09-20, ✅ 已实施并冒烟验证; 未提交; 剩真机目视)**: 新增 `fmtRelTime(ts)`
...
- **时间点列表头右键切换相对/绝对 (FX-28, 2026-09-20, ✅ 已实施并冒烟验证; 未提交; 剩真机目视)**: 覆盖**四个时间点列**
  ...
  ⚠ **已知缺口**: 该键**没有跨标签同步**(列偏好有 `storage` 监听 + `visibilitychange` 补漏, 这个没接)。
