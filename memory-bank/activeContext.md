...
- **① 上轮计划复核的收尾(只剩第 7 项) (2026-09-19)**: 复核报表
  ...
  **剩余两项**(未开工):
  1. **报表 §08 第 7 项 — 节拍对齐**: `sync_interval`(1.5s)与前端分档轮询(1.5/2/3s)在 >3000 种子时错配,
     约一半 `rebuild_views` 无人消费。**需先拍板方向**: 让轮询跟上快档(降 `basePollMs` 下界) vs
     给快照刷新加 Web 活跃门控(无人看就不刷)。⚠ 这条是**频率类**改动, 拍板前先按 pitfalls
     「主循环分层节拍」与「把 main_tick 缩短来换响应速度」两条判据过一遍。
     ⚠ 另注: 第 1 批给 `sync_interval` 加的钳制(`min(sync_interval, main_tick)`)只兜住了"快照新鲜度
     掉到心跳之下", **没有**解决"重建了没人消费"这一半 —— 两件事别混。
  2. **同类端点的同样改法(低优先)**: `/api/torrents/{hash}/files`、`/api/search`、
     `/api/torrents/{hash}/trackers`、`/api/log` 等同样返回裸 dict, 改法一致(`return JSONResponse(...)`)。
     收益取决于响应体大小; 这些是用户触发型、不在 1.5~3s 轮询路径上, 故未纳入本批。
