...
> 最后活动: 2026-09-26 (只读设计审查报告落盘 —— 触发模型/访问节奏/安全审计/翻页模型, 见 reports/26-09-26-1628)
...
- **v3.1+v3.2 扩展配置简化**(2026-09-25/26, clone3): 端点加只读 `GET /api/hr/sites`(每次请求现读配置派生
  `scheme://host/*`, 热重载加站点即生效); 选项页**整页三模板**(极简/标准/完整, 持久化 `uiTemplate`)
  + 站点权限**拉清单勾选 + 一键授权**(手动填域名降级兜底)。明细见档案; 测试 +7, 全量 1617 passed + 1 skipped。

- **v3.4 状态模型重新梳理 + D 档已免罪来源单列**(2026-09-26): 用户指令钉死「最终态 = 已达标 | 未达标 |
  已免罪, 三个状态都代表结束状态」⇒ 终态语义从展示层档案与代码注释收口进主计划 §9 新「状态模型」节
...
- ⛔ **待用户定 (2026-09-26 设计审查)**: 报告 [26-09-26-1628](../reports/26-09-26-1628-report-hr-online-verify-audit.html)
  列 1×P1 + 3×P2 + 3×P3(P1 = 完整刷新判据只有 `header_found`, 三条路径都能让整站误放行; P2 = 放行取消不了 /
  多实例配额翻倍 / 无断点续翻; P3 = 登录失效期照发请求 / token 弱 / 超龄豁免取舍) + §7 用户三案评估
  + §8 **页面排序假设(未验证, 禁止当事实)** 与排序校验告警要求。是否入池或出计划待定。
