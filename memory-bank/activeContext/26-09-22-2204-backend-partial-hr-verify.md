# 部分种子 HR 在线核实
> 摘要: **M1 核心管道 + M2 取数通道 + M3 判定联动 + M4 多站点与打磨均已落地**；只差**真机走查**（M0 四项实测 + 装扩展跑一轮）。
...
> 触发: 部分种子, HR 核实, 浏览器扩展, 三态判定, 带锁访问, 共享站点数据, 转移种子, 多客户端, BTSchool, hr_check, hr_once, hr-status, 取数通道, 本地端点, 取数线程, 告警级别, 站点文件损坏, 隐藏窗口, 多站点, 站点状态, 四类事件, 登录失效
> 最后活动: 2026-09-25 (M4 多站点与打磨)

## 状态

### 已交付（M1 + M2 + M3 + M4 + 四批实报修复）
