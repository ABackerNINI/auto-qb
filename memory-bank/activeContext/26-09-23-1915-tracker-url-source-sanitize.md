...
> 摘要: 把凭据脱敏从"日志出口"提到 **qbapi 拉取即脱敏**。**计划已产出并两轮更新(已入库 `19fa6ee`), 未开工**。三条关键结论: ① 没有代码依赖 `passkey` 值本身, 但有**两处依赖完整 URL**(移除 tracker 的定位键 · 汇报基线当 dict key); ② 顺带查出**第二个泄露面** `GET /api/torrents/{hash}/trackers` 是 qB 透传、完整 URL 直发浏览器(不在 `/api/log`, 上轮守阵扫不到); ③ 用户定了 **W3 收窄为「只保留添加 / 删除」**(编辑下线) + **mask 形态**(保形状 + 值全 hash) + **删除改为「传 hash + mask URL, 当场重取、用完即弃」** —— 原文**不再常驻内存**。
