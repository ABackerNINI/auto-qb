...
- **顺手发现(未修, 范围外)**: `commands run kb.active --check` 被 commands 引擎拒绝(run 无 `<args>` 占位符) —— `_common.GEN_CMD_BY_SCRIPT` 对 `gen_active_recent.py` 的提示指向一条跑不动的命令, 既有缺陷; `kb.baseline` 本轮已按 `<args>` 正确落地。
