# 全量测试耗时归因与优化
> 摘要: 全量 **75s → 默认并行 7.6s**。大头是**环境**(用户调好系统层排除项后四类文件操作全部 <1ms)不是代码; 代码侧另修三处框架开销。**已上并行**(`pytest.ini` 默认 `-n 4` + conftest 台账回传); 代码侧改动已提交推送 `1bde85d`, 并行部分待提交
...
**并行已落地但尚未提交**(`pyproject.toml` / `uv.lock` / `pytest.ini` / `tests/conftest.py`)。
