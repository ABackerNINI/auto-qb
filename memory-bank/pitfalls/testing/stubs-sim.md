...
- **处置**: `scripts/ui_harness.py --hr-site` 注入**真实 HrJudgement** 轮转全分支(含 judged=None 回落),
  ...
  "真对象的非默认形态有没有对应的注入开关"。
- **复发**: 1 —— 2026-09-26 冒烟标签/分类功能时把"`curl /api/tags` 为空"当成功能 bug 排查; 根因即下条
  (泵只写回执不执行 handler)。没命中原因: 不知道泵不执行真实命令 —— 盲区在命令执行层而非替身字段。

### harness 命令泵只写回执**不执行 handler**: 冒烟验证"UI→回执"链路, 不验证"qB 真被改"

- **触发**: 在 `scripts/ui_harness.py` 桩上冒烟**写命令**类功能, 冒烟后想用 `curl /api/*` 或
  页面只读端点核对"后端状态真的变了"。
- **判别**: harness 是桩服务, 没有主循环; `_start_command_pump` 对每条命令按 `--cmd-result`
  模式**直接写 `_web_results`**(ok 模式还补一个延迟真值), **从不调用 `_cmd_*` handler** ⇒
  FakeClient 状态 / store / 全局标签分类列表一概不变。表现为: 前端全链路(toast/胶囊态/回执)
  全部正常, 但 `GET /api/tags`、`/api/categories` 等读端点仍是旧值 —— **这不是 bug**。
- **处置**: 冒烟断言止步于"UI 链路通"(投递/回执/前端状态更新); "命令真的改了 qB"由
  pytest 层的 handler 测试覆盖(替身 calls 断言)。想在桩上验证写后读一致, 得给命令泵加
  handler 执行分支 —— 目前不值得(两类测试分工已覆盖); 排查时先看本文再怀疑功能。
