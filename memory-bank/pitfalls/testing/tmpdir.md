...
> 摘要: 临时目录必须在 `tempfile.gettempdir()` 之下、覆盖率文件不能留仓库根、`TMPDIR` 不设收尾会崩, 别手工加前缀。
> 触发: 跑全量, basetemp, TMPDIR, 临时目录, 覆盖率文件, pytest-current, 耗时, 手工跑子集

### `--basetemp` 换了目录会让 4 条 sidefx 用例**假红**

- **触发**: 想换临时目录跑全量(2026-09-22)。
...
- **判别**: 四条机制 ——
  ...
  · 默认 basetemp 下收尾批量删 `%TEMP%\pytest-of-*` 会撞删除拦截层 ⇒ **点号全过却打不出汇总行**
    (退出码 1, 像失败)。
  · `COVERAGE_FILE` 留在仓库根 ⇒ coverage 开跑时先擦它, 删除同样被拦 ⇒ **开跑 1 秒即退出**。
...
- **判别**: 空载全量 **19~20s**(2026-09-23 末态, 排除项调好之后; 同日更早分别是 54~81s 与 62~114s)——
  **当前基线数字一律以 `testing/baselines/` 最新切片为准**(`commands run kb.baseline`), 此处不复述;
  并发污染下的"某文件 300 秒"是**假数字**(本机放大得特别狠, 见 [perf-measurement.md](perf-measurement.md))。
- **处置**: 直接跑全量。
  ⚠ **别用 `| tail -N` 接 pytest** —— 会缓冲到进程结束才出任何输出, 容易**误判成卡死**。

### 全量 pytest 报 `PermissionError: … pytest-current` 是临时目录被污染, **不是测试红**

- **触发**: 全量 pytest 打完点号后报错(2026-09-21)。
...
- **处置**: **不要去删那些目录**(同样会被拦), 给 pytest 指一个全新临时根即可(现口径更简单:
  直接走 `commands run test.*`, `TMPDIR` 已内置)。
...
- **判别**:
  - **现象**: `uv run pytest tests -q` 打完所有点(如 `[100%]`)之后抛
    ...
    **注意测试本身是过的**, 崩在 `pytest_sessionfinish` 对 `pytest-current` 符号链接做
    `.resolve().exists()`(机制见上条 2026-09-21 条目)。
  - **改 `H:\Temp` 的目录权限无效**(用户改过, 仍然红); 关沙箱跑同样如此 ⇒ **不是工具沙箱**,
    是那个盘上的**重解析点读取被拒**(该链接能 stat 能列, `readlink` / `unlink` 全被拒 —— 用户态修不掉)。
...
- **处置**(两档可用解, 都实测过): 跑之前把 `TMPDIR` 指到**不靠重解析点**的盘 ——
  ...
  WinError 4390, `cleanup_dead_symlinks` 没东西可读, 反倒绕开崩溃); 两者全量均全过。
  用户自己终端(`TMPDIR` 在 C 盘)无此问题。
  ⇒ 工具 shell 跑全量前**先设 `TMPDIR`**; 见到这个 `PermissionError` **先怀疑临时目录, 别当代码回归**。
  📌 **2026-09-22 已固定为 `TMPDIR="R:/Temp/auto-qb/tests"`**(R 盘不支持符号链接, 反而绕开了这个崩溃),
  完整约定见 [../../techContext.md](../../techContext.md)「临时目录 / 备份盘约定」。
  📌 **2026-09-23 已写进闸门命令(真正的收口)**: `.commands/my-commit-flow/.my-commit-flow.toml` 里那条 pytest 闸门改成
