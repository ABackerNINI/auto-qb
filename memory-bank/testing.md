...
> prism/atlas 双 UI 断言(当前 **72 项 0 失败**(ok 模式; 单 UI 各 36 项)/
> **72 项 0 失败**(`--expect-cmd error`, 回滚路径; 单 UI 各 36 项)/
...
> **P0-3 值覆盖及时落回真值**(量 `pendingOps` 归零, 80~1000ms **带下界**, 见下方"真值对齐"条目)、
> **P0-3 压暗在回执后及时撤下**(量 DOM `.is-pending` 消失, <250ms; 2026-09-22 拆出来的那一半)、
...
> ✅ **2026-09-22 已修: 双 UI 那 2 条恒红是**断言量错了对象**, 不是代码回归** ——
...
>   ⚠ **②的上界 250 是量出来的、不是拍的**: 压暗不结束时 DOM 的 `.is-pending` 会一直挂到**值覆盖
>   释放**才消失(~350ms) ⇒ 上界取 400 会让它**擦线过关**(实测 349/374ms), 压到 250 才必红;
>   正常路径 ~20ms(12 倍余量)。❗该值随库规模浮动(refresh 越慢退化值越大), 3000 种子是定阈值口径。
>   **红绿双验**: ①注入"回执即 `delete pendingOps`"(值覆盖不过真值) ⇒ 20ms < 80 ⇒ 双 UI 变红;
>   ②注释掉 `op.grey = false`(压暗不随回执结束) ⇒ 384/376ms > 250 ⇒ 双 UI 变红; 两条撤回后全绿。
>   ⚠ 顺带发现: 单改 `_optimisticSettled` 恒真(2026-09-19 那条"拿自己的补丁当真值比对"的回归形态)
>   **打不红①** —— 桩里 value 覆盖的释放被 refresh 时刻门槛住(真值 120ms 时才到, 首个 refresh 已在
>   378ms), 两种实现都落在同一刻 ⇒ 那条形态实际由「真值事件后不被陈旧快照打回」守着(红验中它确实变红)。
