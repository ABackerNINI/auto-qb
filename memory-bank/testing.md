...
> ⚠ **2026-09-21 实测: 58 项 / 失败 2 项**(两 UI 各一条 `P0-3 乐观态及时落回真值(80~1000ms)` —— 实测
>   pending 只活 19~21ms, **低于断言下界 80ms**, 即断言自己说的"没等真值、跟自己的补丁比上了")。
>   **该失败与当轮改动无关**: 把被测表临时改回旧值重跑, 失败项**完全相同**(3 次运行 3 次复现)。
>   项数 56 → 58 疑为**条件断言**(按流程/数据决定是否执行)的差异, 不是脚本新增。**本轮未修**(属既有问题;
>   待定: 是真值对齐被回退了, 还是下界 80ms 已随"撤下"优化过时 —— 后者的话该改断言而不是改代码)。
