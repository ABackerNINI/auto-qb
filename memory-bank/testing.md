...
8. **测试期禁止真实系统通知** (2026-09-18 新增, 两层防护): 全量测试实测会**真的发系统通知**(本机抓到一条 Windows toast `auto-qb 已停止`)。
