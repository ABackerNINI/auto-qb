...
- **`localStorage` 两种"重置"必须分清, 且不要用"升版本"应对列集变更** (R10-09):
  ...
  (`LEGACY_COLS_KEYS`, 按列 key 洗净后回写当前键)。
  ③ **多标签页整份覆盖 = 用户说的"列设置经常被重置"** (issue 26-09-20-1800, 真浏览器复现, **已修**):
  内存是**页面加载时读一次的快照**(`initialColState`), 而 `saveColState()` 写的是**整份**
  `{widths,hidden,manual,order}` ⇒ **last-writer-wins**: 谁最后动一下, 存储就变成谁的快照,
  先改的那个标签的改动被**静默吞掉**(不报错、不提示)。修法是 **F1 写入改 read-modify-write**
  (读回存储为底, 只覆盖本次涉及的那个 page) **+ F2 监听 `storage` 事件跨标签 adopt**
  + F3 `visibilitychange` 回到可见时补一次 —— F1 单独**不够**: 用户两个标签改的通常是**同一个表**,
  page 级合并在同表冲突时仍然后写赢, 必须让每个标签的内存保持新鲜。
  **判别法: 报"偏好/设置被重置"时先问"几个标签页"** —— 单标签复现不出来就别怀疑存储本身。
  换密钥/登出只清 `autoqb_token`, 全仓无
...
### 前端轮询间隔: 必须按种子量分档, 且档位要实测 (2026-09-19)

- **⚠ 顺序不能反**: "降低轮询间隔"会**放大**"全量回传 + 整树重渲染"的频率 —— 在 P1-1(按视图回传)
  与 P1-2(行窗口化)落地**之前**动它, 在大库上是**加剧**不跟手而不是缓解。计划里刻意把它排在最后。
- **⚠ 档位要量不要拍**: 用桩服务 A/B 量出"窗口化后单轮 refresh 的真实耗时", 再按
  **主线程占用率 ≈15%** 反推间隔。实测: 1000 种子 143ms / 3000 种子 353ms / 5000 种子 ~550ms
  ⇒ 档位 **≤1000 → 1.5s / 1000~3000 → 2s / >3000 → 3s**(占用率 10% / 15% / 17%)。
- **⚠ 下界是服务端节拍, 不是 0**: 服务端每 `sync_interval`(1.5s)才刷一次数据, 前端轮询快过它
  只会多拿一次"版本未变"的空响应(响应体趋近于零, 但不产生新数据) ⇒ **1.5s 就是有意义的下界**。
- **⚠ 别把"按种子量分档"和"无变化退避"混为一谈**: 后者按 rid 是否变化放慢, 已经踩过一次坑
  (状态栏走 /api/stats 不受门控 ⇒ 变成"状态栏正常、种子行滞后")。分档只跟**总量**有关,
  跟本轮有没有变化无关, 不会引入那类错位。
- 分档后 `pollSec` 数据字段就成了没人读的死字段 —— **删掉**, 别留着当"兜底"(实际不会被走到)。
...
- **⚠ 桩服务是"瞬时"的 ⇒ 顺序型 / 延迟型缺陷本地永远测不出来**(2026-09-19, issue 26-09-19-1939):
  冒烟的 `FakeClient` 与命令泵回执都是毫秒级, 于是"`applyOptimistic()` 写在 `await POST` 之后"这种
  **写错行序**的缺陷, 在"点击后立即可见 pending"这条断言下一路 PASS —— 直到真机大库把 POST 拖到
  2-4s, 用户肉眼才报"点了半天才变"(受控复现: 注入 2000ms 时补丁 **2012ms** 才贴)。
  **判据**: 凡是"某段代码必须在某次 IO 之前/之后发生"的约束, 断言必须**人为造慢**才有意义 ——
  `page.route(CMD_URL)` 注入 800ms, 再断言点击 → `.is-pending` < 400ms(红绿双验实测 823ms / 5ms)。
  两个踩过的坑: ① Playwright 新版路由谓词收到的是 **URL 对象**不是字符串(`u.includes` 直接
  `TypeError`, 先 `String(u)`); ② 点击时刻要取**菜单项的 click 事件**(捕获阶段), 用 Playwright 的
  `h.click()` 时刻含鼠标开销, 实测虚高 ~250ms, 会把"14ms"读成"265ms"而误判成渲染慢。
- **顺带一条同类判据: 真机上"慢"必须能自己说话**。`cmdStats` 曾只量回执段(`wait_ms`/`exec_ms`),
  "点击 → 命令投递"是盲区 ⇒ 用户报 2-4s 时日志全绿、无从下手。前端埋点要覆盖**整条链路**
  (补丁 / POST / 排队 / 执行 / 端到端), 且必须有 `[perf]` 消费者 —— 埋点没有消费者就是死字段。
...
### 改 `_tick` / `_sync_line` / `_task_line` 签名时, 必须同步**所有**测试替身 —— 否则整轮测试会卡死 (2026-09-19 实测)

- **⚠ 症状极难定位**: 给 `_tick`/`_sync_line`/`_task_line` 加了参数后, 全量 `pytest` 从 28s 变成
  **卡死 10 分钟以上**(我用 `timeout` 才拦下来), 且**不报失败** —— 卡在某处, `-v` 只显示最后一条
  PASSED 在 85% 附近。单跑 `test_qbmanager.py` 反而只报 2 条 FAILED, 看起来像小问题。
- **根因**: 测试里这些方法是被**本地替身**顶掉的(`mgr._tick = tick` / `lambda` / `Mock(side_effect=…)`),
  替身签名没跟着加参数 ⇒ 主循环每轮调用都抛 `TypeError` ⇒ 被 `run()` 的兜底 `except` 吞掉并继续下一轮
  ⇒ **退出条件(比如替身里到点 raise KeyboardInterrupt)永远不会被触发** ⇒ 死循环。
  ⇒ 于是"签名不匹配"被放大成"整个测试会话挂起", 而不是一条清楚的失败。
- **判别法**: 全量跑突然变慢/卡住, 且 `-v` 停在某个跑 `mgr.run()` 的用例附近 ⇒ 先怀疑最近是不是改了
  主循环方法的签名。用 `grep -rn "_tick = \|_sync_line = \|_task_line = " tests/` 找齐所有替身(注意替身
  **不一定叫 tick**, 也可能是 `def boom(dry_run):` 这种局部函数 —— 只 grep `def tick(` 会漏)。
- **修法**: 新参数一律给默认值(`force: bool = False`), 并把上条 grep 找到的替身**全部**补上。

### 服务端节拍与客户端节拍错配: 用 rid 的消费情况反向门控(方案 B, 2026-09-19)

- 服务端 `sync_interval` 固定 1.5s 重建, 前端 `basePollMs()` 按种子量 1.5/2/3s ⇒ >3000 种子时每 3s 产 2 版
  而客户端只取最后 1 版, 中间那版的重建 CPU 无人消费(实测 20 周期: 40 次 → **20 次, 省 50%**)。
- **修法**: `_web_pending_ver` 记"已发布但还没被任何 `/api/state` 请求取走的版本号"; `_flush_views()` 只在
  没有欠账时重建。`rebuild_views()` 发布时登记, `ensure_group_state()` 观察到当前版本时清空(拿到数组 或
  被告知"你已是最新" 都算已消费)。
- **⚠ 两个必须守住的边界**: ① **脏标记必须保留**(只是不生产, 不是丢弃变化) —— 客户端一取走立刻补上;
  ② **`force=True` 必须绕过** —— 本轮有命令改了种子状态时, P0-5 要求真值几十毫秒内进快照, 被门控挡住
  就等于把用户操作的可见性退回到"等下一次轮询", 与 P0-5 相悖。
...
- **判别法(务必先做)**: 起桩服务后先确认它服务的就是本工作区的文件:
  ...
  ```
  为 0 就是连错人了。**顺手再对一下字节数/改动时间** —— 本轮就是这样发现"服务的文件比磁盘上的新",
  从而定位到是别人在同时改同一个文件。
- **修法**: 用自己的专属端口(如 8111 / 8112), 别抢 8099; 并在跑冒烟前做上面那一次 curl 校验。
- 附带: 用 `&` 起的后台桩服务会随命令结束被回收, 要长期跑请用后台任务方式, 否则跑到一半
  会出现 `net::ERR_CONNECTION_REFUSED`(冒烟的 reload 那一步最容易撞上)。

### 冒烟断言别用「固定睡 N 毫秒再采样」—— 要等条件成立 (2026-09-19 实测, 两种假象)

- **⚠ 两种假象, 方向相反**: ① **假失败** —— 动作规模大时(整剧操作会把该剧**全部**成员一起补丁,
  桩里一"剧"就有 1500 个种子), `applyOptimistic` 是逐 hash 扫表, 补丁贴完之前 `pendingOps`
  还没填齐 ⇒ 150ms 采样看到"还没有 is-pending"; ② **假阳性** —— 补丁现在贴在 POST **之前**
  (issue 26-09-19-1939 的修法), 失败要等回执回来才回滚 ⇒ 150ms 采样看到"还没回滚的 is-pending"
  (本轮 atlas 在 error 模式就抓到过)。
- **修法**: 断言改成**等条件成立**再判 —— 成功路径 `waitForFunction` 等目标 class 出现;
  失败路径 `waitForFunction` 等 `pendingOps` 归零, 再断言"不留假状态"。给一个合理超时
  (成功 1.5s / 失败 4s)即可, 仍然比固定采样快且稳。
- **教训**: 凡是"异步贴完才可见"的状态, 采样时点本身就是一个隐藏的失败源; 而且**规模不同
  时点就不同** —— 桩里的大剧和真机上的小剧差一个数量级, 轮询把两种都覆盖到。

### 推送之后要回扫一遍「未提交」—— 文档状态比代码更容易过期 (2026-09-19, 同一个坑踩了两次)

- **场景**: 修完先在 `activeContext.md` / `progress.md` / issue 报告里写「已修未提交」(这是对的,
  写的时候确实还没提交), 提交推送后**没人回头改** ⇒ 文档长期宣称"未提交", 而主线里早就有了。
  同一天里因此多出一笔纯文档提交两次(`41a38c1`、`9e8061b` 之后的又一笔)。
- **做法**: 推送完成后 `grep -rn "未提交\|尚未提交" memory-bank/` 扫一遍, 把本次自己的条目改成
  **「已入库 `<sha>`」**(带提交号, 比"已提交"可核验)。状态变更日志里也一并改成 `已入库 <sha>`。
- ⚠ **只改自己这一轮的**: 并发会话的条目同样会失效(比如对方推了 `fed62a8` 但文档还写"尚未提交"),
  但那归对方会话 —— 按 scope-guard, 不要顺手替别人改, 否则 rebase 必然撞车。
- 同理适用于**基线数字**: 改了测试就两侧(Windows / WSL)都重测再落, 只测一侧就更新必漂移。

### ⚠ 分支落后主线时, 先 `git fetch` 再动手 —— 否则会把别人已修好的问题重做一遍 (2026-09-19 实测)

- **事故**: 在落后主线 4 个提交的分支上, 独立实现了「乐观态挂满 3s 才消除」的完整修复(真值匹配即清 +
  回执后拉真值 + 桩服务真改状态), 验证也全绿; 拉齐时才发现 `origin/develop` 上已有**同一问题的独立
  修复**(`32f531d` + `12657ee`), 思路几乎一致。**一整轮实现白做**, 而且两套写法并存会撕裂维护。
- **更危险的**: 主线那一版里还含一处我**没有**的修复 —— `12657ee`「3s 兜底超时必须显式回滚 `op.prev`」:
  `isPending()` 超时只 `delete pendingOps[hash]` 的话, 在 rid 门控下服务端版本未变时不回传数组、
  行对象不被替换 ⇒ **补丁值永久留在行上**(命令根本没执行, 界面却一直显示已暂停, 违反红线)。
  我的版本有同样缺陷, 而我的 hang 冒烟只查 `pendingOps === 0`、**没查最终状态色**, 所以没暴露。
- **判别法**: 开工前 `git fetch origin develop && git status -sb`; 看到 `[behind N]` 就先读那 N 个提交
  的 `--stat` 再决定做什么。协作主线在 Gitee(见 AGENTS.md), **不要用 GitHub 的提交状态判断进度** ——
  它只是镜像、允许滞后。
- **真有重叠时的处理**(本次做法): ① 落后分支的工作先 `git stash push -u` 留档; ② rebase 到主线;
  ③ 逐项比对, 只把**主线没有的增量**重新实现一遍(本次保住三处: `settleMs` 埋点、桩服务自愈回弹、
  静态守阵); ④ 被取代的那份计划文档**必须加存档声明**, 否则文档会描述一套代码里不存在的实现。

### ⚠ 桩服务"真的改状态"之后必须自愈, 且回弹不能是盲定时 (2026-09-19 实测)

- **背景**: `scripts/ui_harness.py` 的命令泵原本**只写回执、不改状态** ⇒ 服务端真值永远不变 ⇒
  "真值对齐即清 pending"这类断言只会测到"走满 3s", 跟没测一样。主线已让 `pause/resume` **真改**
  `tor.state` 并 `rebuild_views()`(还刻意"先回执、后改状态"复刻真机补刷新的错位)。
- **⚠ 副作用**: 状态真的永久生效后, **长驻**桩服务会跨轮累积 —— 跑过一次"整剧暂停"(合成数据里
  一剧 = 全部 800 个种子)之后, 第二轮冒烟**所有行都是 s-paused**, 4 条"挑一个未暂停的行"的断言
  全部假失败, 而它们与被测代码毫无关系。加 `--state-revert-ms`(默认 1500)自愈。
- **❗回弹必须是「真值被 `/api/state` 取走之后」再等 N ms, 不能是「生效后 N ms」的盲定时**:
  实测定时 4s 回弹把一次整组撤下拖到 4149ms —— 回弹跑到了前端观测之前, 把真值又改回去了,
  前端要等下一次回弹才碰巧对上。判"已被取走"用 `_web_pending_ver` 被清空(它在 `ensure_group_state`
  里被清)。回弹记的是**最初**值(不是上一次命令后的值), 否则 pause→resume 连续两次会把种子
  还原成暂停态。
- **同族第三坑(与主线修复同一轮撞到)**: 真值变快之后, "点一下 → 睡 120/150ms → 看有没有
...
### ⚠ "服务端的值"不等于"已落地的值": 拿它覆盖本地预测会显示错误状态 (2026-09-20 实测回归)

- **事故**: 为了让乐观态快点撤下, 改成"回执自带真值, 且与预测不一致时**采纳真值**"。
  理由听起来很硬: 真值是补刷新之后从 qB 读的, 是权威值。真机立刻打脸 ——
  点「开始」→ 乐观做种 0.x 秒 → **弹回「已暂停」** → 约 2 秒后才真正变做种。
- **根因**: `torrents/resume` 返回 200 时 **qB 还没翻状态**, 紧跟着的补刷新读到的仍是
  **命令前**的 `paused`。于是"权威值"其实是个**滞后一拍的旧值**, 采纳它 = 把行改成错误状态。
- **判别**: 与本地预测不一致的服务端值, 绝大多数是**还没落地**, 不是"预测猜错了"。
  **"权威" ≠ "已落地"**。想区分只有两条路: ① 服务端等它落地再发(`_truth_landed()` +
  上限兜底); ② 前端不采纳、继续等。别猜。
- **修法**: 前端只在真值**匹配**时撤下, 不一致就保留乐观值继续等; 服务端在发回执前**等真值落地**
  (上限 1200ms, 超时必须照发 —— 否则前端 waitCmd 干等 40s); 等待期间按 0.2s 快速补刷新,
  不要干等一个 `sync_interval`(真机大库 2s, 那正是用户抱怨的"2 秒")。
- **代价排序(重要)**: **显示错误状态 >> 多等一会儿**。为了省 1.5s 的等待而引入显示错误状态的
  分支, 是笔完全不划算的买卖 —— 这是我这次亲手犯的错, 由真机反馈才暴露(本地仿真端翻状态是
  瞬时的, 根本复现不出这个滞后)。 88fdc44 (知识库回写: "权威 ≠ 已落地" 坑 + 当前焦点定案)

## 预检脚本的 STOP 级别要按阶段区分（2026-09-20，my-commit-flow dogfooding 时抓到）

- 症状: 预检把"落后主线 N 个提交"一律判 STOP, 结果**连本地提交都被挡住**; 而正确顺序恰恰是
  "先本地提交 → 工作区干净 → 才能安全 rebase → 才能推"。一刀切的 STOP 挡住了安全动作,
  反而逼执行者绕过脚本(`--skip-preflight`), 护栏就没了。
- 修正: 预检加 `--phase commit|push` —— **commit 阶段落后只 WARN**(本地提交可以, 推送前必须 rebase),
  **push 阶段才 STOP**。
...
- **2026-09-20 19:1x 补测(同一天第二次踩, 结论要加两条)**:
  ① **它是间歇性的, 不是"偶发一次"** —— 同一条 `git push gitee develop`(当时已是 no-op)连测 5 次**只成功 2 次**;
     且**失败与成功交替出现**, 与命令写法无关(手工跑、脚本 `subprocess` 非 TTY 跑、关 sandbox 跑都一样)。
  ② **命令报失败 ≠ 没推上** —— 本次 `push.py` 退 5(判"主线推送失败")并因此**没试镜像**, 但 `git ls-remote`
     显示远端**早已是本地 HEAD**: 早先某次重试其实成功了, 失败的输出是**之后那一次**的。
     ⇒ **判定"推没推上"的唯一依据是 `git ls-remote <远端> <分支>`(它自己也会瞬时失败, 要重试 2~3 次再下结论)**;
     `git push --dry-run` **不能**用来判断(只做 ref 协商, 不发包, 永远"成功")。
  ③ 因此 `push.py` 现在的缺口(待改进, 本轮未改): 主线 push 命令失败退 5 之前, 应先 `ls-remote` 复核一次 ——
     若远端已 == HEAD 就该继续走镜像, 而不是判失败。手工补法:
     `git -c http.https://github.com.proxy= push github develop`(镜像仍只尝试一次)。
  ④ 反过来也成立: **别因为看到 Recv failure 就认定"我的命令把 git 弄崩了"** —— 查的办法是同一条命令连跑几次看成功率,
     而不是改命令。Git 侧的崩溃会写 `fatal:` 之外的栈/退出码异常, 单纯 `Recv failure` 是链路层。
...
## 把「问答」当「执行任务」一路做到推送 (2026-09-20 用户指定红线)

- **现象**: 用户只问"本项目能启用 Playwright 吗", 代理一路做到入池 issue + 认领 + 出计划文档 + 4 次推送, 最后全部 revert。
- **根因**: 把"我提议 → 用户说继续"当成了全权授权; 又拿「收尾 DoD / 立档阈值 / 入池」三条规则给自己加码 —— **那三条的前提都是"正在执行任务"**。
- **判据**: 开工先判这一轮是**问答 / 只读**还是**执行任务**。用户只是问("能不能 X""是什么""怎么看""有没有") ⇒ **只在回复里作答**; 想延伸排查(跑测试、跑冒烟、写探针、做实验)**先问**, 别把"回答"做成"交付"。
- ❗**"继续 / continue / 接着做 / 你看着办"不构成授权** —— 只表示"把你手上这一步做完", 不表示"可以开始下一件更大的事"。四类动作必须**逐项**拿到显式确认: ①新建文件(计划文档 / 任务档案 / 报告) ②入池 issue ③认领 issue(置 In Progress) ④commit / push。

## ⚠️ 跨视图的常驻消费者不能依赖按视图裁剪的阵列 (2026-09-20, 状态栏速度恒为 0; 同类第二次)

- 现象: WEBUI 状态栏的下载/上传速度在**种子页恒为 0**。数据源与刷新链路都是好的 —— 辅种页显示正常。
- 根因: P1-1「按视图回传」把 `groups` 从种子页裁掉 (`VIEW_ARRAYS["torrent"] = ("torrents",)`),
  而前端契约是「键不存在则保留原引用」(`app.js:883`, 这条本身**正确**, 否则没回的视图会被抹空),
  于是 `this.groups` 在种子页上永远停在初始 `[]` ⇒ 状态栏对 `groups` 的求和恒 0。
- **这是该类错误的第二次**: 第一次是 2026-09-19 的 BUG-8 —— 追剧页只回 `shows`, 而成员索引是
  `groups + singles` 拼的 ⇒ 索引为空、整页永久空白。共同模式: **常驻/跨视图的消费者去读一个
  按视图裁剪的数据源**, 而裁剪者不知道有这个消费者。
- 修法方向: 这类"全局聚合"一律**服务端算好放进 `status` 恒回传**(与 `traffic` / `server` 同口径),
  不参与 `VIEW_ARRAYS` 分片、不参与 `rid` 门控; 前端只读这个标量。**不要**反过来把阵列加回
  `VIEW_ARRAYS` —— 那会废掉 P1-1 的体积优化(种子页 4.52MB → 6.41MB), 且守阵
  `test_api_state_view_scoped_payload` 明确断言 `"groups" not in t`。
- 排查判据: 每次往 `VIEW_ARRAYS` 增删键、或新增一个"常显"UI 元素(状态栏/顶栏计数/徽标)时,
  都要问一句「它的数据源会不会在某个视图下不回传?」。
- 复验手法(可复用, 比静态阅读快且不给假结论): 起
