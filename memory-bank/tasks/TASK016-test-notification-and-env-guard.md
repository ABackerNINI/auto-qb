...
| 25 | 文档漂移(新发现, 待定) | 全量扫 30 个 md: **另有 27 处** `docs/*.html` 旧路径缺失(`modules.md` + TASK001~011)。**属独立清理任务, 未自动改** —— 见"待用户决定" | ⏸ |
...
- 2026-09-18: **文档漂移**(AGENTS.md 要求"发现漂移时回写")。
  ...
  **实际有 5 处不是 2 处**: tracker-group-plan / webui-naming-plan / webui-redesign-plan /
  ...
  会让本次提交文件数从 9 涨到 ~18, 且其中 `docs/dependency-lock-report.md/.html` 是**既有的畸形引用**
  (需要人判断原意)。已列入"待用户决定"。
- 2026-09-18: 全量 **1018 passed / 0 failed**(基线 1007 + 11)。**未提交**。
