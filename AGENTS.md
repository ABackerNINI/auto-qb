...
- **先合并远端, 再收尾** (2026-09-26 定稿, 治「baseline 总是撞」): 收尾回写 (baseline.md / activeContext 切片 / 各 _index) 必须落在**合并远端之后**的新基线上 —— 预检落后即按「同步路径」合并, 再进入收尾; 回写文件**随主提交一并暂存**, 不推完再补一笔 (已推送的提交不能 amend + 强推)。
