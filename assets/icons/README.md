...
| 位置 | 来源 | 说明 |
|------|------|------|
| `src/auto_qb/web_ui/static/icon.png` (256, favicon) | 由 `auto-qb-mono-06-orbit.svg`(轨道方案)等比栅格化 | 深蓝圆角底 `#16294A` + 330° 白色轨道弧 + 绿色种子点 `#34C759` + 白色 qb 字形(`q/b` 圆碗描边 40、b 竖笔在 x=282); 生成脚本为一次性草稿(未入库), 重生成时按同一几何比例 512→256 缩放即可 |
| 顶栏/登录卡 `#i-orbit` sprite symbol | 同上方案的**矢量简写**(弧 + 绿点 + 中心圆) | 与 favicon 同源, 保证"页签图标 = 页内品牌图标"一致; 改其一时另一处同步改 |
