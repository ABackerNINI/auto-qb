...
> 只有站点侧**压根没有可依据的数据**(该站未接入 / `mode: off` / 取数线程还没发布第一版视图)时才
> 回落本地的 `downloaded` 口径 —— 这层兜底是有意的: 把「没数据」当「未核实」处理, 会让 `mode: all`
> 站点在启动窗口里让整站种子集体触发打标。WebUI 详情能看到三态、依据与两边数值对照。
...
只读现状(同样不写盘 / 不连 qB / **不取数**): 把**已落盘**的数据摊开给你核对 —— 档位分布、每行下载量与
剩余达标、放行记录数、配额与熔断状态、最近一次刷新的结果与原因, 以及最需要人眼对账的**明细表**
(`tid / 档位 / 下载量 / 剩余达标 / 名称 / infohash`):
